...
export class Inspector {
  ...
  public static inspect(
      dbName: string|null, tableName: string|null, limit?: number,
      skip?: number): string {
    if (dbName === null) {
  ...
  private static listTables(dbName: string): string {
    ...
    if (global !== undefined && global !== null) {
      ...
      global.getService(Service.SCHEMA).tables().forEach((table) => {
  ...
  private static inspectTable(
      dbName: string, tableName: string, limit?: number,
      skip?: number): string {
    const global = Inspector.getGlobal(dbName);
    let contents: object[] = [];
    if (global !== undefined && global !== null) {
      ...
      if (table !== undefined && table !== null) {
        ...
        if (rowIds.length) {
          contents =
              (cache.getMany(rowIds) as Row[]).map((row) => row.payload());
        }
      }
    }
