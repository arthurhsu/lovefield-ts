...
export class JoinStep extends PhysicalQueryPlanNode {
  ...
  constructor(
      global: Global, readonly predicate: JoinPredicate,
    ...
    this.algorithm = this.predicate.evaluatorType === EvalType.EQ ?
        JoinAlgorithm.HASH :
        JoinAlgorithm.NESTED_LOOP;
    this.indexJoinInfo = null as any as IndexJoinInfo;
  }
    ...
    switch (this.algorithm) {
      ...
      case JoinAlgorithm.INDEX_NESTED_LOOP:
        return [this.predicate.evalRelationsIndexNestedLoopJoin(
            relations[0], relations[1], this.indexJoinInfo, this.cache)];
      default:  // JoinAlgorithm.NESTED_LOOP
