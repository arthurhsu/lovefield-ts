...
describe('EndToEndFKUpdateCascade', () => {
  ...
  it('update1FK', async () => {
    ...
    await db.update(tA)
        .set(tA['id'], updatedId)
        .where(tA['id'].eq(sampleRows.tableA[0].payload()['id']))
        .exec();

    tx = db.createTransaction();
    const results = await tx.exec([
      db.select().from(tA).where(tA['id'].eq(updatedId)),
      db.select().from(tB).where(tB['foreignId'].eq(updatedId)),
    ]);
  ...
  it('update2FK', async () => {
    ...
    await db.update(tA)
        .set(tA['id1'], updatedId1)
        .set(tA['id2'], updatedId2)
        .where(op.and(
            tA['id1'].eq(sampleRows.tableA[0].payload()['id1']),
            tA['id2'].eq(sampleRows.tableA[0].payload()['id2'])))
        .exec();

    tx = db.createTransaction();
    const results = await tx.exec([
      db.select().from(tA).where(
          op.and(tA['id1'].eq(updatedId1), tA['id2'].eq(updatedId2))),
      db.select().from(tB).orderBy(tB['id']),
    ]);

    assert.equal(1, results[0].length);
    assert.equal(3, results[1].length);
    assert.equal(updatedId1, results[1][0]['foreignId1']);
    assert.equal(updatedId2, results[1][0]['foreignId2']);
    assert.equal(updatedId2, results[1][1]['foreignId2']);
  });
