...
describe('Runner', () => {
  ...
  it('transaction_Read', () => {
    ...
    const queryTask1 =
        new MockTask(TransactionType.READ_WRITE, createScope(), () => {
          executionOrder.push('query1');
        }, TaskPriority.USER_QUERY_TASK);
    const queryTask2 =
        new MockTask(TransactionType.READ_ONLY, createScope(), () => {
          executionOrder.push('query2');
        }, TaskPriority.USER_QUERY_TASK);

    const promises = [queryTask1, queryTask2].map(
        (queryTask) => runner.scheduleTask(queryTask));
  ...
  it('transaction_Write', () => {
    ...
    const queryTasks = ids.map((id) => {
      ...
      const queryTask =
          ...
          }, TaskPriority.USER_QUERY_TASK, name);
  ...
  it('task_Success', () => {
    ...
    const queryTask = new MockTask(
        TransactionType.READ_WRITE, createScope(), () => expectedResult,
        TaskPriority.USER_QUERY_TASK);

    return runner.scheduleTask(queryTask).then(
        (result) => assert.equal(expectedResult, result as any as string));
  });
  ...
  it('task_Failure', () => {
    ...
    return runner.scheduleTask(queryTask).then(
        assert.fail, (error) => assert.deepEqual(expectedError, error));
  });
  ...
  it('scheduleTask_Prioritize', () => {
    ...
    const task1 = new MockTask(
        TransactionType.READ_WRITE, createScope(), () => resolver.promise,
        TaskPriority.USER_QUERY_TASK);
    const task2 =
        new MockTask(TransactionType.READ_WRITE, createScope(), () => {
  ...
  it('transaction_WriteWhileReading', () => {
    ...
    return Promise.all(promises).then(() => {
      ...
      assert.sameOrderedMembers(
          ['q1 start', 'q1 end', 'q2 start', 'q2 end'], executionOrder);
    });
  });
});
