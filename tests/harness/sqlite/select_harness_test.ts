...
describe('DeleteHarness', () => {
  ...
  function checkFlatten(
      expected: string, rows: object[], fields: string[],
      tablePrefix?: string): void {
    const actual =
        rows.map((obj) => {
              const objToCheck = tablePrefix ? obj[tablePrefix] : obj;
              assert.equal(fields.length, Object.keys(objToCheck).length);
              return fields.map((name) => objToCheck[name].toString())
                  .join(' ');
            })
            .join(' ');
  ...
  it('Select1_1', async () => {
    ...
    // 1-1.4
    let results: object[] = await db.select(t1['f1']).from(t1).exec();
    checkFlatten('11', results, ['f1']);

    // 1-1.5
    results = await db.select(t1['f2']).from(t1).exec();
    checkFlatten('22', results, ['f2']);

    // 1-1.6, 1-1.7
    results = await db.select(t1['f1'], t1['f2']).from(t1).exec();
    checkFlatten('11 22', results, ['f1', 'f2']);

    // 1-1.8
    results = await db.select().from(t1).exec();
    checkFlatten('11 22', results, ['f1', 'f2']);
    ...
    // 1-1.9
    results = await db.select().from(t1, t2).exec();
    ...
    // 1-1.10
    results = await db.select(t1['f1'], t2['r1']).from(t1, t2).exec();
    checkFlatten('11', results, ['f1'], 't1');
    checkFlatten('1.1', results, ['r1'], 't2');

    // 1-1.11
    results = await db.select(t1['f1'], t2['r1']).from(t2, t1).exec();
    checkFlatten('11', results, ['f1'], 't1');
    checkFlatten('1.1', results, ['r1'], 't2');

    // 1-1.11.1
    results = await db.select().from(t2, t1).exec();
    checkFlatten('11 22', results, ['f1', 'f2'], 't1');
    checkFlatten('1.1 2.2', results, ['r1', 'r2'], 't2');

    // 1-1.11.2
    results = await db.select().from(t1.as('a'), t1.as('b')).exec();
