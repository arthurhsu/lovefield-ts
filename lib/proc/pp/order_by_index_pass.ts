...
export class OrderByIndexPass extends RewritePass<PhysicalQueryPlanNode> {
  ...
  private applyTableAccessFullOptimization(orderByStep: OrderByStep):
      PhysicalQueryPlanNode {
    ...
    if (tableAccessFullStep !== null) {
      ...
      rootNode = TreeHelper.replaceNodeWithChain(
                     tableAccessFullStep, tableAccessByRowIdStep,
                     indexRangeScanStep) as PhysicalQueryPlanNode;
    }
  ...
  private applyIndexRangeScanStepOptimization(orderByStep: OrderByStep):
      PhysicalQueryPlanNode {
    let rootNode: PhysicalQueryPlanNode = orderByStep;
    ...
    if (indexRangeScanStep !== null) {
      ...
      rootNode =
          TreeHelper.removeNode(orderByStep).parent as PhysicalQueryPlanNode;
    }
  ...
  private findTableAccessFullStep(rootNode: PhysicalQueryPlanNode):
    ...
    const tableAccessFullSteps =
        TreeHelper.find(rootNode, filterFn, stopFn) as TableAccessFullStep[];
  ...
  private findOrderByStep(
      rootNode: PhysicalQueryPlanNode, queryContext: SelectContext): OrderByStep
      |null {
    ...
    return TreeHelper.find(
               rootNode, (node) => node instanceof OrderByStep)[0] as
        OrderByStep;
  }
  ...
  private getIndexCandidateForIndexSchema(
    ...
    const columnsMatch = indexSchema.columns.length === orderBy.length &&
        orderBy.every((singleOrderBy, j) => {
          ...
          return singleOrderBy.column.getName() ===
              indexedColumn.schema.getName();
        });
  ...
  private checkOrder(orderBy: SelectContextOrderBy[], indexSchema: Index):
    ...
    const ordersRightBitmask =
        indexSchema.columns.reduce((soFar, indexedColumn) => {
