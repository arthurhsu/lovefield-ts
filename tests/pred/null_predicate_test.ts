...
describe('NullPredicate', () => {
  ...
  it('isNull_NonIndexed', async () => {
    ...
    assert.equal(0, tableSchema['datetime'].getIndices().length);
    ...
    assert.equal(
        sampleData[1].payload()['string'], retrievedEmployee['string']);
  });
  ...
  it('isNotNull_NonIndexed', async () => {
    ...
    const retrievedEmployee = result[0];
    assert.equal(
        sampleData[0].payload()['string'], retrievedEmployee['string']);
  });
  ...
  it('isNull_Indexed', async () => {
    ...
    assert.isTrue(tableSchema['id'].getIndices().length >= 1);
