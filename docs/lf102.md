...
const res2 = await db
  ...
  .from(c, o)
  .where(
    lf.op.and(c.col('id').eq(o.col('customerId')), o.col('customerId').eq(1))
  )
  .orderBy(o.col('date'))
  .exec();
...
either succeed together (called **commit**), or fail together (called
**rollback**). If the transaction failed, data remained in their original state
untouched.
...
async function setVacationsForNewEmployees(startDate) {
  ...
  const q1 = db
    .select(e.col('id'))
    .from(e)
    .where(e.col('hireDate').gt(startDate));
...
db.update(c).set(c.col('name'), 'Bar').where(c.col('id').eq(1)).exec();
...
```javascript
schemaBuilder
  .createTable('OnlineTransactions')
...
- `RESTRICT`<br>
  ...
  that violated the constraint. **Parent** table is the referenced table, and
