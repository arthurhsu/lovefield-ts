...
describe('Journal', () => {
  ...
  it('noWriteOperations', () => {
    const table = env.schema.table('tableC');
  ...
  it('insert_New', () => {
    ...
    const pkIndex =
        env.indexStore.get(pkIndexSchema.getNormalizedName()) as RuntimeIndex;
    const rowIdIndex =
  ...
  it('insert_PrimaryKeyViolation_SingleColumn', () => {
    ...
    const primaryKey = '100';
    const row = table.createRow({id: primaryKey, name: 'DummyName'});
    const otherRow = table.createRow({id: primaryKey, name: 'OtherDummyName'});
  ...
  it('insert_PrimaryKeyViolation_CrossColumn', () => {
    const table = env.schema.table('tableD');
    const id1 = 'DummyId';
    const id2 = 100;
  ...
  it('insert_UniqueKeyViolation_SingleColumn', () => {
    ...
    journal.insert(table, [row1]);
    journal.commit();
    assert.isTrue(emailIndex.containsKey(row1.payload()['email']));
    ...
    assert.equal(row1.id(), emailIndex.get(row1.payload()['email'])[0]);
    assert.isTrue(rowIdIndex.containsKey(row1.id()));
    assert.equal(1, env.cache.getCount());
  });
  ...
  it('insert_NotNullableKeyViolation', () => {
    ...
    TestUtil.assertThrowsError(
        ...
        ErrorCode.NOT_NULLABLE, journal.insert.bind(journal, table, [row1]));
    journal.rollback();
    assert.equal(0, env.cache.getCount());
  });
  ...
  it('update_NotNullableKeyViolation', () => {
    ...
    journal = createJournal([table]);
    const rowUpdatedValid =
        table.createRow({id: 'pk1', email: 'otherEmailAddress'});
  ...
  it('update_NoPrimaryKeyViolation', () => {
    ...
    journal.commit();
    assert.equal(1, env.cache.getCount());
    assert.isTrue(pkIndex.containsKey(row.payload()['id']));
    assert.isTrue(rowIdIndex.containsKey(row.id()));
    ...
    assert.isFalse(pkIndex.containsKey(row.payload()['id']));
    assert.isTrue(pkIndex.containsKey(rowUpdated2.payload()['id']));
  });
  ...
  it('update_PrimaryKeyViolation1', () => {
    ...
    assert.isTrue(pkIndex.containsKey(row1.payload()['id']));
    assert.isTrue(pkIndex.containsKey(row2.payload()['id']));
  ...
  it('update_PrimaryKeyViolation3', () => {
    ...
    rows.forEach(
        (row) => assert.isTrue(pkIndex.containsKey(row.payload()['id'])));
  ...
  it('update_updateNullsCorrectly', () => {
    const table = env.schema.table('tableI');
    ...
    let updatedRow =
        table.createRow({id: 'pk0', id2: 'id2x', name: 'sampleName'});
    updatedRow.assignRowId(row.id());
    journal = createJournal([table]);
  ...
  it('insertOrReplace_UniqueKeyViolation', () => {
    ...
    journal.commit();
    assert.isTrue(emailIndex.containsKey(row1.payload()['email']));
    assert.isTrue(emailIndex.containsKey(row2.payload()['email']));
    ...
    journal.rollback();
    assert.equal(row1.id(), emailIndex.get(row3.payload()['email'])[0]);
    ...
    journal.rollback();
    assert.isFalse(emailIndex.containsKey(row4.payload()['email']));
    assert.isFalse(emailIndex.containsKey(row5.payload()['email']));
    ...
    journal.rollback();
    assert.equal(row1.id(), emailIndex.get(row1.payload()['email'])[0]);
    assert.equal(row2.id(), emailIndex.get(row2.payload()['email'])[0]);
    ...
    assert.equal(2, env.cache.getCount());
    assert.equal(row1.id(), emailIndex.get(row1.payload()['email'])[0]);
    assert.equal(row2.id(), emailIndex.get(row2Updated.payload()['email'])[0]);
  });
  ...
  it('indicesCacheUpdated', () => {
    const table = env.schema.table('tableB');
    ...
    const pkId =
        env.indexStore.get(indices[0].getNormalizedName()) as RuntimeIndex;
    const idxName =
        env.indexStore.get(indices[1].getNormalizedName()) as RuntimeIndex;
    const rowIdIndex =
        env.indexStore.get(table.getRowIdIndexName()) as RuntimeIndex;
    ...
    const assertIndicesUpdated = () => {
      ...
      assert.sameDeepMembers(
          [3, 1], pkId.getRange([new SingleKeyRange('3', '4', false, false)]));
  ...
  it('indicesUpdated', () => {
    ...
    journal.insert(table, [row3]);
    const index =
        env.indexStore.get(indexSchema.getNormalizedName()) as RuntimeIndex;
  ...
  it('rollback', () => {
    ...
    const rowToModifyNew =
        table.createRow({id: 'modify', name: 'UpdatedDummyName'});
    ...
    const assertInitialState = () => {
      ...
      assert.isTrue(pkIndex.containsKey(rowToModifyOld.payload()['id']));
      assert.isTrue(pkIndex.containsKey(rowToRemove.payload()['id']));
      assert.isFalse(pkIndex.containsKey(rowToInsert.payload()['id']));
