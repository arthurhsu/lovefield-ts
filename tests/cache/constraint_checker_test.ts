...
describe('ConstraintChecker', () => {
  ...
  it('findExistingRowIdInPkIndex', async () => {
    ...
    const pk1 = row1.payload()['id'];
    const pk2 = row2.payload()['id'];
  ...
  // constraint timing.
  async function checkForeignKeysForInsert(constraintTiming: ConstraintTiming):
      Promise<void> {
  ...
  async function checkForeignKeysForDelete(constraintTiming: ConstraintTiming):
      Promise<void> {
    const schema1 = SchemaTestHelper.getOneForeignKey(constraintTiming);
    await setUpEnvForSchema(schema1);
  ...
  async function checkForeignKeysForUpdate(constraintTiming: ConstraintTiming):
      Promise<void> {
    const schema1 = SchemaTestHelper.getOneForeignKey(constraintTiming);
    await setUpEnvForSchema(schema1);
    const parentTable = env.schema.table('Parent');
  ...
  it('detectCascadeDeletion_TableChain', async () => {
    ...
    await setUpEnvForSchema(schema1);
    const tableA = env.schema.table('TableA');
    ...
    assert.sameDeepOrderedMembers(
        [tableB.getName()], cascadedDeletion.tableOrder);
  ...
  it('detectCascadeDeletion_TwoForeignKeys', async () => {
    const schema1 =
        SchemaTestHelper.getTwoForeignKeys(ConstraintAction.CASCADE);
    await setUpEnvForSchema(schema1);
    const tableA = env.schema.table('TableA');
  ...
  function assertChecks(
      ...
      checkFn: (timing: ConstraintTiming) => void): void {
    TIMINGS.forEach((timing) => {
      if (timing === constraintTiming) {
        // Foreign key constraint violation on constraint {0}.
        TestUtil.assertThrowsError(
            ErrorCode.FK_VIOLATION, checkFn.bind(null, timing));
      } else {
