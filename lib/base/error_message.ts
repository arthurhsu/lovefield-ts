...
class ErrorMessages {
  ...
  private static getInstance(): ErrorMessages {
    ErrorMessages.instance = ErrorMessages.instance || new ErrorMessages();
...
function setupMap(): Map<ErrorCode, string> {
  ...
  m.set(
      ec.INVALID_TX_ACCESS, 'Attempt to access in-flight transaction states.');
  m.set(ec.OUT_OF_SCOPE, 'Attempt to access {0} outside of specified scope.');
  m.set(
      ec.INVALID_TX_STATE, 'Invalid transaction state transition: {0} -> {1}.');
  m.set(ec.INCOMPATIBLE_DB, 'Attempt to open a newer database with old code.');
  m.set(
      ec.ROW_ID_EXISTED,
  ...
  m.set(ec.FK_VIOLATION, 'Foreign key constraint violation on constraint {0}.');
  ...
  m.set(
      ec.BIND_ARRAY_OUT_OF_RANGE, 'Cannot bind to given array: out of range.');
  m.set(
  ...
  m.set(ec.FK_COLUMN_NONUNIQUE, 'Foreign key {0} refers to non-unique column.');
  ...
  m.set(ec.NULLABLE_PK, 'Primary key column {0} cannot be marked as nullable');
  m.set(
      ec.DUPLICATE_NAME,
