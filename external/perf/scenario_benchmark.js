...
export class ScenarioBenchmark {
  ...
  async insertTxAttach() {
    ...
    const fn = () => {
      ...
      return tx
        .attach(this.db.insert().into(this.brand).values([row]))
        .then(fn);
    };
  ...
  getTestCases() {
    return [
      new TestCase('Insert via Tx Attach', this.insertTxAttach.bind(this)),
      new TestCase('Select', this.select.bind(this)),
      new TestCase('Select Binding', this.selectBinding.bind(this)),
      new TestCase('Teardown', this.tearDown.bind(this), undefined, true),
    ];
  }
}
