...
describe('DeleteHarness', () => {
  ...
  function checkFlatten(
      expected: string, rows: object[], fields: string[]): void {
    const actual =
        rows.map((obj) => {
              assert.equal(fields.length, Object.keys(obj).length);
              return fields.map((name) => obj[name].toString()).join(' ');
            })
            .join(' ');
  ...
  it('Delete3', async () => {
    ...
    let results: object[] = await db.select().from(t1).orderBy(t1['f1']).exec();
    checkFlatten('1 2 2 4 3 8 4 16', results, ['f1', 'f2']);

    // 3.1.2
    await db.delete().from(t1).where(t1['f1'].eq(3)).exec();

    // 3.1.3
    results = await db.select().from(t1).orderBy(t1['f1']).exec();
    ...
    // 3.1.6
    await db.delete().from(t1).where(t1['f1'].eq(2)).exec();
  ...
  it('Delete5', async () => {
    ...
    // 5.1.2
    let results: object[] = await db.select(fn.count()).from(t1).exec();
    ...
    await db.insert().into(t1).values(rows).exec();
    results = await db.select(fn.count()).from(t1).exec();
    ...
    // 5.4.1
    await db.delete().from(t1).where(t1['f1'].gt(50)).exec();
    ...
    await Promise.all(promises);
    results = await db.select(t1['f1']).from(t1).orderBy(t1['f1']).exec();
    checkFlatten(
    ...
    // 5.7
    await db.delete().from(t1).where(t1['f1'].neq(48)).exec();
    results = await db.select(t1['f1']).from(t1).orderBy(t1['f1']).exec();
