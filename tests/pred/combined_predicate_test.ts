...
describe('CombinedPredicate', () => {
  ...
  it('copy_Simple', () => {
    ...
    const original = op.and(
      e.col('salary').gte(200),
      e.col('salary').lte(600)
    ) as PredicateNode;
  ...
  it('copy_VarArgs', () => {
    ...
    const original = op.and(
      e.col('salary').gte(200),
      e.col('salary').lte(600),
      e.col('commissionPercent').lt(0.15),
      e.col('commissionPercent').gt(0.1)
    ) as PredicateNode;
    const copy = original.copy() as PredicateNode;
    assertTreesIdentical(expectedTree, original, copy);
    assert.equal(original.getId(), copy.getId());
  ...
  it('getColumns', () => {
    ...
    let expectedColumns = [e.col('salary')];
    ...
    expectedColumns = [e.col('salary'), e.col('commissionPercent')];
  ...
  it('setComplement_Or', () => {
    const predicate = op.or(e.col('salary').lte(200), e.col('salary').gte(600));
  ...
  it('isKeyRangeCompatible_Or', () => {
    const keyRangeCompatiblePredicates = [
      op.or(e.col('salary').eq(200), e.col('salary').eq(600)),
    ...
    const notKeyRangeCompatiblePredicates = [
      op.or(e.col('firstName').match(/Foo/), e.col('firstName').eq('Bar')),
      op.or(e.col('firstName').neq('Foo'), e.col('firstName').eq('Bar')),
      op.or(
        e.col('salary').eq(100),
        op.or(e.col('salary').eq(200), e.col('salary').eq(300))
      ),
  ...
  it('toKeyRange_Or', () => {
    const testCases = [
      ...
      [
        op.or(e.col('salary').lt(200), e.col('salary').lt(100)),
        '[unbound, 200)',
      ],
