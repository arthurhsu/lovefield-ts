...
export class MultiColumnOrPass extends RewritePass<PhysicalQueryPlanNode> {
  ...
  public rewrite(rootNode: PhysicalQueryPlanNode, queryContext: Context):
    ...
    do {
      ...
      indexRangeCandidates =
          this.findIndexRangeCandidates(orSelectStep, queryContext);
    ...
    return TreeHelper.find(
               ...
               (node) =>
                   (node instanceof TableAccessFullStep &&
                    node.table.getName() === tableName))[0] as
        TableAccessFullStep ||
        null;
  }
  ...
  private findIndexRangeCandidates(
      ...
      queryContext: Context): IndexRangeCandidate[]|null {
    const predicate =
        queryContext.getPredicate(selectStep.predicateId) as PredicateNode;
    ...
    const allIndexed = predicate.getChildren().every((childPredicate) => {
      const indexRangeCandidate = indexCostEstimator.chooseIndexFor(
          queryContext, [childPredicate as PredicateNode]);
  ...
  private replaceWithIndexRangeScan(
      selectStep: SelectStep, tableAccessFullStep: TableAccessFullStep,
      indexRangeCandidates: IndexRangeCandidate[]): PhysicalQueryPlanNode {
    ...
    indexRangeCandidates.forEach((candidate) => {
      const indexRangeScanStep = new IndexRangeScanStep(
          this.global, candidate.indexSchema, candidate.getKeyRangeCalculator(),
          false /* reverseOrder */);
